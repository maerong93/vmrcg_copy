...
@include_once($dddd2_skin_path.'/write_comment_update.head.skin.php');
...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    $sn_objectName = $_POST['sn_objectName'];
    $wr_subject = get_text(stripslashes($wr['wr_subject']));
    $sql = " insert into $write_table
                set                     
                     sn_subject = '{$sn_subject}',
                     sn_objectName = '{$sn_objectName}',
...
$sql_scene = "select * from g5_write_free where wr_id =$wr_id";

$result_scene = sql_query($sql_scene);
$row_scene = sql_fetch_array($result_scene);
$sn_scene = $row_scene['sn_scene'];

   $sql_qa = "insert into scenario_qa(sc_content,wr_id,qa_id,sn_in_datetime,sn_scene) values('{$sc_content}','{$wr_id}','{$qa_id}','".G5_TIME_YMDHIS."','{$sn_scene}')";  

$result_qa = sql_query($sql_qa);
$row_qa = sql_fetch_array($result_qa);


// 사용자 코드 실행
@include_once($dddd2_skin_path.'/write_comment_update.skin.php');
@include_once($dddd2_skin_path.'/write_comment_update.tail.skin.php');
delete_cache_latest($bo_table);

header("location:board.php?bo_table=free&wr_id={$wr_id}");
//$redirect_url = short_url_clean(G5_HTTP_VMRCG2_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr['wr_parent'].'&amp;'.$qstr.'&amp;#c_'.$comment_id);
