...
if ($config['cf_editor'] && $is_dhtml_editor_use && $board['bo_use_dhtml_editor'] && $member['mb_level'] >= $board['bo_html_level']) {
    ...
    if ( $w == 'u' && (! $is_member || ! $is_admin || $write['mb_id'] !== $member['mb_id']) ){
        ...
        $content = get_text(html_purifier($write['sn_explanation']), 0);
    }
...
$editor_html = editor_html('sn_explanation', $content, $is_dhtml_editor);
$editor_js = '';
$editor_js .= get_editor_js('sn_explanation', $is_dhtml_editor);
$editor_js .= chk_editor_js('sn_explanation', $is_dhtml_editor);

// 임시 저장된 글 수
$autosave_count = autosave_count($member['mb_id']);

include_once(G5_PATH.'/head.sub.php');
@include_once ($dddd2_skin_path.'/write.head.skin.php');
include_once('./board_head.php');

$action_url = https_url(G5_VMRCG2_DIR)."/write_update.php";

echo '<!-- skin : '.(G5_IS_MOBILE ? $board['bo_mobile_skin'] : $board['bo_skin']).' -->';
@include_once ($dddd2_skin_path.'/write.skin.php');

include_once('./board_tail.php');
@include_once ($dddd2_skin_path.'/write.tail.skin.php');
