...
if($page_rows > 0) {
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        if ($is_search_bbs)
            ...
            $row = sql_fetch(" select * from {$write_table} where sn_no = '{$row['wr_parent']}' ");
        $list[$i] = get_list($row, $board, $board_skin_url, G5_IS_MOBILE ? $board['bo_mobile_subject_len'] : $board['bo_subject_len']);
...
if ($member['mb_level'] >= $board['bo_write_level']) {
    $write_href = short_url_clean(G5_VMRCG_URL.'/write.php?bo_table='.$bo_table);
}
...
if ($board['bo_use_rss_view']) {
    $rss_href = G5_VMRCG_URL.'/rss.php?bo_table='.$bo_table;
}
