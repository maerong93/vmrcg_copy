...
if ($sca || $stx || $stx === '0') {     //검색이면
    ...
    // $sql = " SELECT COUNT(*) AS `cnt` FROM g5_write_free ";
    // $row = sql_fetch($sql);
    // $total_count = $row['cnt'];
...
$total_pages = ceil($total_count / $config['cf_write_pages']);

g5_latest_cache_data($board['bo_table'], $list);
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], 
                $page, $total_pages, get_pretty_url($bo_table, '', $qstr.'&amp;page='));
                //$page, $total_page, './board.php?bo_table='.$bo_table.$qstr.'&page=');
                //1,10,'list.php');
                //$total_page  = ceil($total_count / $page_rows);  // 전체 페이지 계산
...
if ($board['bo_use_rss_view']) {
    $rss_href = G5_VMRCG2_URL.'/rss.php?bo_table='.$bo_table;
}
