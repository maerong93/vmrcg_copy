...
@include_once($dddd2_skin_path.'/write_update.head.skin.php');
...
// }
$wr_seo_title = exist_seo_title_recursive('vmrcg2', generate_seo_title($wr_subject), $write_table, $wr_id);
...
if ($w == '' || $w == 'r') {
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($write['mb_id'])
            alert('로그인 후 수정하세요.', G5_VMRCG2_URL.'/login.php?url='.urlencode($return_url));
    }
...
// 사용자 코드 실행
//@include_once($dddd2_skin_path.'/write_update.skin.php');
@include_once($dddd2_skin_path.'/write_update.tail.skin.php');

delete_cache_latest($bo_table);

$redirect_url = run_replace('write_update_move_url', short_url_clean(G5_HTTP_VMRCG2_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr), $board, $wr_id, $w, $qstr, $file_upload_msg);
