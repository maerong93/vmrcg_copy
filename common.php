...
define('G5_HTTP_VMRCG2_URL',  https_url(G5_VMRCG2_DIR, false));
define('G5_HTTPS_VMRCG2_URL', https_url(G5_VMRCG2_DIR, true));
...
if (isset($_REQUEST['sn_no'])) {
    $sn_no = (int)$_REQUEST['sn_no'];
} else{
    $sn_no = 0;
}

if(isset($_REQUEST['sn_scene'])) {
    $sn_scene = (int)$_REQUEST['sc_scene'];
} else{
    $sn_scene = 0;
}
...
if (isset($_REQUEST['jsonName'])) {
    if (!is_array($_REQUEST['jsonName'])) {
        $jsonName = preg_replace('/[^a-z0-9_]/i', '', trim($_REQUEST['jsonName']));
        //$jsonName = $_REQUEST['jsonName'];
    }
} else {
    $jsonName = '';
}
...
//===================================
// aside show hide 구분 (json 폴더인 경우에는 안보이게끔 처리), 기본값은 true(보임)
$is_aside_view = "display:block;"; 
//===================================
...
if (G5_IS_MOBILE) {
    ...
    $dddd_skin_path   = get_skin_path('dddd', 'theme/basic');
    $dddd_skin_url    = get_skin_url('dddd', 'theme/basic');

    $dddd2_skin_path   = get_skin_path('dddd2', 'theme/basic');
    $dddd2_skin_url    = get_skin_url('dddd2', 'theme/basic');
    ...
    // json 스킨 {
    $json_skin_path      = get_skin_path('json', 'theme/basic');
    $json_skin_url       = get_skin_url('json', 'theme/basic');
    $json_left_menu_path       = get_skin_path('json', 'theme/basic');
    $json_left_menu_url       = get_skin_url('json', 'theme/basic');
    // } json 스킨 
}
//==============================================================================
